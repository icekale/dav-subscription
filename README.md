...
- **抓取策略**：**雪球组合最高频（30s，空轮封顶 2min）** + 优先大V短间隔（默认 60s）+ 普通大V全局间隔（默认 180s），随机抖动与失败退避；**无新帖自动降频**（空轮间隔 2 倍步进，普通大V封顶 900s，有新帖立即恢复），轮内随机错峰打破固定节律，X 降级 RSSHub 期间限速；组合调仓**实时推送**（不走合并摘要）；微博 cookie 自动续期保活（登录失败 30 分钟冷却），雪球 cookie 定时探测失效并告警（雪球首页续期已被反爬接管，需手动更新 cookie）
